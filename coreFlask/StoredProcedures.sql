...
DELIMITER //
CREATE PROCEDURE GetDoctorsOfHospital
(IN hospital VARCHAR(30))
BEGIN
  SELECT * FROM Doctors WHERE Hospital=hospital;
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE GetDonationByBlood
(IN blood VARCHAR(4))
BEGIN
  SELECT * FROM Donations WHERE Blood=blood ORDER BY Date DESC ;
END //
DELIMITER ;
